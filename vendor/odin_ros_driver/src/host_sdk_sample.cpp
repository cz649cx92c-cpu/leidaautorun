...
static void lidar_device_callback(const lidar_device_info_t* device, bool attach)
{
	...
	std::string config_dir = "";
        ...
        if (g_custom_map_mode == 1) {
            ...
            // Reset the one-shot success log gate for each relocalization session.
            // Without this, a long-lived driver process only reports
            // "relocalization success!" once after boot, and upper layers that
            // watch the log can wait forever on later relocalization attempts.
            g_relocalization_success_msg_printed = false;
